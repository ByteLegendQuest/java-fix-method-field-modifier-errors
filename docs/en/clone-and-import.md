...
`git clone https://github.com/ByteLegendQuest/java-fix-method-field-modifier-errors`

A project directory named `java-fix-method-field-modifier-errors` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-method-field-modifier-errors` directory which is created just now.
